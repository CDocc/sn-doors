...
local function isAuthorized(Player, door, usedLockpick)
	...
	if Config.AdminAccess and BJCore.Functions.HasPermission(Player.PlayerData.source, Config.AdminPermission) then
...
BJCore.Functions.CreateCallback('sn-doors:server:setupDoors', function(_, cb)
	cb(Config.DoorList)
end)

BJCore.Functions.CreateCallback('sn-doors:server:checkItems', function(source, cb, items, needsAll)
...
RegisterNetEvent('sn-doors:server:updateState', function(doorID, locked, src, usedLockpick, unlockAnyway, enableSounds, enableAnimation, sentSource)
	...
	TriggerClientEvent('sn-doors:client:setState', -1, playerId, doorID, locked, src or false, enableSounds, enableAnimation)
	...
	SetTimeout(Config.DoorList[doorID].autoLock, function()
		...
		if Config.DoorStates[doorID] == nil then Config.DoorStates[doorID] = locked elseif Config.DoorStates[doorID] ~= locked then Config.DoorStates[doorID] = nil end
		TriggerClientEvent('sn-doors:client:setState', -1, playerId, doorID, true, src or false, enableSounds, enableAnimation)
	end)
end)

RegisterNetEvent('sn-doors:server:saveNewDoor', function(data, doubleDoor)
	local src = source
	if not BJCore.Functions.HasPermission(src, Config.CommandPermission) and not IsPlayerAceAllowed(src, 'command') then
		if Config.Warnings then
			showWarning(Lang:t("general.warn_no_permission_newdoor", {player = GetPlayerName(src), license = BJCore.Functions.GetIdentifier(src, 'license'), source = src}))
		end
		return
	end
	...
	TriggerClientEvent('sn-doors:client:newDoorAdded', -1, configData, identifier, src)
end)
...
RegisterNetEvent('sn-doors:server:removeLockpick', function(type)
	local Player = BJCore.Functions.GetPlayer(source)
...
BJCore.Commands.Add('newdoor', Lang:t("general.newdoor_command_description"), {}, false, function(source)
	TriggerClientEvent('sn-doors:client:addNewDoor', source)
end, Config.CommandPermission)

BJCore.Commands.Add('doordebug', Lang:t("general.doordebug_command_description"), {}, false, function(source)
	TriggerClientEvent('sn-doors:client:ToggleDoorDebug', source)
end, Config.CommandPermission)
